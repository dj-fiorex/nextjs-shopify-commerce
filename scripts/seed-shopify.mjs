...
const FEATURED_COLLECTION = "hidden-homepage-featured-items";
const CAROUSEL_COLLECTION = "hidden-homepage-carousel";
...
async function ensureProductImages() {
  const data = await gql(
    `{ products(first: 25, query: "status:active") { nodes { id title handle featuredMedia { id } } } }`
  );
  const missing = data.products.nodes.filter((p) => !p.featuredMedia);
  if (!missing.length) {
    console.log("All products already have images.");
    return;
  }

  console.log(`Attaching images to ${missing.length} product(s)...`);
  for (const product of missing) {
    ...
    const result = await gql(
      ...
      }`,
      {
        ...
        media: [
          { originalSource: url, alt: product.title, mediaContentType: "IMAGE" },
        ],
      }
    );
    const errors = result.productCreateMedia.mediaUserErrors;
    if (errors?.length) {
      console.warn(`  ! ${product.handle}: ${errors[0].message}`);
    } else {
      console.log(`  + image queued for ${product.handle}`);
    }
  }
...
async function ensureProducts(publications) {
  const existing = await gql(
    `{ products(first: 25, query: "status:active") { nodes { id title handle } } }`
  );
...
async function ensureCollection(handle, title, productIds, publications) {
  const existing = await gql(
    `query find($q: String!) {
  ...
  if (!collection) {
    const data = await gql(
      ...
      }`,
      {
        input: {
          title,
          handle,
          descriptionHtml: `<p>Products shown in the ${title.toLowerCase()} section of the homepage. Collections prefixed with "hidden-" are excluded from the search page.</p>`,
          products: productIds,
        },
      }
    );
    ...
    console.log(`+ created collection: ${handle} (${productIds.length} products)`);
  } else {
    ...
    if (missing.length) {
      ...
      console.log(`= collection exists: ${handle} — added ${missing.length} product(s)`);
    } else {
